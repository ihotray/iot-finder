...
local function discovery(req)
    ...
    if res == nil or res.type == nil or res.type ~= 'static' or res.address == nil or res.address == "" then
        ...
        if handle == nil then
            return cjson.encode({ code = -1, type = 'offer', message = 'failed to get address' })
        end
        ...
        for _, line in pairs(lines) do
            if string.find(line, inet_prefix) then
                ...
                return cjson.encode({ code = 0, type = 'offer', message = 'success', data = { address = ip_address } })
            end
        end
    else
        --static
        return cjson.encode({ code = 0, type = 'offer', message = 'success', data = { address = res.address } })
    end

    return cjson.encode({ code = -1, type = 'offer', message = 'no address found' })
end


local function offer(req)
    -- set handle offer to do something
    return cjson.encode({ code = 0, type = 'ack', message = 'success' })
end
